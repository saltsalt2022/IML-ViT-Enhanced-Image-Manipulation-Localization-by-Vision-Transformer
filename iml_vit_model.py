...
class iml_vit_model(nn.Module):
    ...
    def forward(self, x:torch.Tensor, masks, edge_masks, shape= None):
        ...
        mask_logits, class_feature_logits = self.predict_head(feature_list)
        class_logits=F.sigmoid(class_feature_logits)
        # up-sample to 1024x1024
        mask_pred = F.interpolate(mask_logits, size = (self.input_size, self.input_size), mode='bilinear', align_corners=False)
        ...
        #法一
        # 根据 masks 是否为空生成 0 或 1 的标签
        device = class_logits.device  # 获取 class_logits 所在的设备

        class_label = torch.tensor([1] if masks.max().item() != 0 else [0], dtype=torch.float16).to(device)  # device 是当前模型所在的设备
        class_logits=F.sigmoid(class_feature_logits)
        class_loss = self.BCE_loss(class_logits,class_label ) 
        ...
        # masks_original=masks[:, :, :shape[0][0],: shape[0][1]]
        # print(f"masks_original.shape:{masks_original.shape}")
        # # 打印最大值和最小值
        # print(f"Max value of masks: {masks.max().item()}，Min value of masks: {masks.min().item()}")
        # print(f"Max value of masks_original: {masks_original.max().item()}，Min value of masks_original: {masks_original.min().item()}")
        # print(f"masks.mean(dim=(1, 2, 3):{masks.mean(dim=(1, 2, 3))}")
        # print(f"masks_original.mean(dim=(1, 2, 3):{masks_original.mean(dim=(1, 2, 3))}")
        # class_loss = self.BCE_loss(class_logits, masks_original.mean(dim=(1, 2, 3))) if masks is not None else 0  # 平均值近似全局真伪
        # print(f"class_logits:{class_logits}")
        # print(f"class_class_feature_logits:{class_feature_logits}")
        
        
        predict_loss += edge_loss
        predict_loss += class_loss
        mask_pred = torch.sigmoid(mask_pred)
        
        return predict_loss, mask_pred, edge_loss,class_logits,class_loss
