...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, (samples, masks, edge_mask, shape) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            predict_loss, predict, edge_loss, class_logits,class_loss= model(samples, masks, edge_mask, shape)
        ...
        class_loss_reduce = misc.all_reduce_mean(class_loss_value)
        if log_writer is not None and (data_iter_step + 1) % 50 == 0:
            ...
            log_writer.add_scalar('train_loss/class_loss', class_loss_reduce, epoch_1000x)
    ...
    if log_writer is not None:
        ...
        log_writer.add_scalar('train/class_logits', class_logits, epoch)
...
def test_one_epoch(model: torch.nn.Module,
    ...
    with torch.no_grad():
        ...
        for data_iter_step, (images, masks, edge_mask, shape) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
            ...
            predict_loss, predict, edge_loss ,class_logits,class_loss= model(images, masks, edge_mask)
            # 打印 class_logits 的值
            print(f"Step {data_iter_step}: Class logits = {class_logits.cpu().numpy()}")
            print(f"Step {data_iter_step}: Class logits shape = {class_logits.shape}, values = {class_logits.cpu().numpy()}")
