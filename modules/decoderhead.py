...
class PredictHead(nn.Module):
    def __init__(self, 
        ...
        self.global_classifier = nn.Sequential(
            ...
            nn.Flatten(),
            nn.Linear(embed_dim, 1)  # Single classification output
        )
    def forward(self, x):
        ...
        # Pixel-level prediction
        mask_logits = self.linear_predict(x)

        # Image-level classification
        class_logits = self.global_classifier(x)
        class_logits=class_logits.squeeze(dim=-1)
