...
def load_model(args, model_without_ddp, optimizer=None, loss_scaler=None):
    if args.resume:
        ...
        # 检查 checkpoint 是否包含完整的状态（包括 model, optimizer 等）
        if 'model' in checkpoint:
            model_state_dict = checkpoint['model']
            print("Loading 'model' from checkpoint.")
        else:  # 直接假定 checkpoint 是模型的 state_dict
            model_state_dict = checkpoint
            print("Loading checkpoint as pure 'state_dict'.")

        # 加载模型权重
        model_without_ddp.load_state_dict(model_state_dict, strict=False)
        print(f"Resume checkpoint {args.resume} successfully.")

        # 仅在 checkpoint 包含 optimizer 和 scheduler 时加载它们的状态
        if optimizer and loss_scaler:
            if 'optimizer' in checkpoint and 'epoch' in checkpoint and not (hasattr(args, 'eval') and args.eval):
                ...
                print("Optimizer and scheduler states not found in checkpoint.")
